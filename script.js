// PART 1: SIMULATED STUDENT ARRAY (Import Placeholder)
// NOTE: Replace this with the actual 100-element array provided in the link.
const students = [
...
const genderTablesContainer = document.getElementById('genderTablesContainer');
const femaleTableBody = document.getElementById('femaleTableBody');
const maleTableBody = document.getElementById('maleTableBody');

// --- PART 3: Render Function (Mapping) ---
function renderTable(data, tbodyElement) {
  tbodyElement.innerHTML = ''; // Clear existing rows

  if (data.length === 0) {
    tbodyElement.innerHTML = '<tr><td colspan="7">No students found.</td></tr>';
    return;
  }
  ...
  data.forEach(student => {
    ...
    // Passing Status = "passing" or "failed"
    const passingStatus = student.passing ?
      `<span class="passing-status passing">Passing</span>` :
      `<span class="passing-status failed">Failed</span>`;

    const row = document.createElement('tr');
    row.innerHTML = `
            <td>${student.id}</td>
            <td class="name-cell">
                <img src="${student.img_src}" alt="${fullName}" class="student-image">
                <span>${fullName}</span>
            </td>
            <td>${student.class}</td>
            <td>${student.marks}</td>
...
function handleSearch() {
  ...
  // Hide gender tables if visible
  tableContainer.style.display = 'block';
  genderTablesContainer.style.display = 'none';

  if (searchTerm === '') {
    // If search is cleared, render the default/last sorted data
...
function sortData(criteria) {
  // Start with a fresh copy of the initial data for sorting
  let sortedData = [...initialStudents];

  // Hide gender tables and show main table by default
  tableContainer.style.display = 'block';
  genderTablesContainer.style.display = 'none';
  ...
  switch (criteria) {
    ...
    // Sort A->Z (Ascending by Full Name)
    case 'az':
      sortedData.sort((a, b) => {
    ...
    // Sort Z->A (Descending by Full Name)
    case 'za':
      sortedData.sort((a, b) => {
        const nameA = `${a.first_name} ${a.last_name}`.toUpperCase();
        const nameB = `${b.first_name} ${b.last_name}`.toUpperCase();
        return nameB.localeCompare(nameA); // Reverse comparison
      });
      break;
    ...
    // Sort by Passing (Filter Only Passing)
    case 'passing':
    ...
    // Sort by Gender (Display Two Tables)
    case 'gender':
      // Hide the main table and show the gender containers
      tableContainer.style.display = 'none';
      genderTablesContainer.style.display = 'block';

      const femaleData = sortedData.filter(student => student.gender === 'Female');
      const maleData = sortedData.filter(student => student.gender === 'Male');

      renderTable(femaleData, femaleTableBody);
      renderTable(maleData, maleTableBody);
      // Exit function as rendering is already done for gender tables
      return;
  ...
  // Update the global students array for the next operation (like another sort/search)
  // NOTE: For 'default' and 'gender' cases, we rely on the initial array or return early.
  if (criteria !== 'default') {
    students.splice(0, students.length, ...sortedData); // Update students array in place
  } else {
    students.splice(0, students.length, ...initialStudents);
  }

  // Render the resulting data in the main table
  renderTable(students, tableBody);
}


// Initial load: Render the table with the default order
window.onload = function () {
  renderTable(initialStudents, tableBody);
}